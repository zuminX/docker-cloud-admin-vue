<template>
  <table v-loading="loading" class="ui striped table">
    <thead>
...
export default {
  ...
  methods: {
    ...
    async updateCurrentPageData(page = 1, pageSize = 5) {
      const { success, data } = await loadingRequest(this.requestUserListByMode(page, pageSize), this)
      if (success) {
